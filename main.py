...
def main():
    parser = argparse.ArgumentParser(description="HFGA-TS SMT Scheduling System CLI Runner")
    parser.add_argument(
        "--file", 
        type=str, 
    ...
    parser.add_argument("--gen", type=int, default=100, help="GA Maximum Generations (default 100 for fast CLI)")
    parser.add_argument("--split", type=float, default=150.0, help="Batch Splitting Threshold T")
    parser.add_argument("--transport", type=float, default=10.0, help="Transit time between workstations (min)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for enrichment")
    parser.add_argument("--sequential", action="store_true", help="Disable multiprocessing and run sequentially")
    ...
    print(f"  • Preprocessing split: {len(ga_engine.batches)} total batches generated from {len(problem.jobs)} jobs.")
    ...
    def progress_log(generation, best_fitness, average_fitness, diversity, pc, pm):
        if generation % 10 == 0 or generation == 1 or generation == args.gen:
            print(f"  [Gen {generation:4d}/{args.gen}] Best Fit (Tardiness): {best_fitness:8.1f} min | Avg: {average_fitness:8.1f} min | Div: {diversity:5.3f} | Pc: {pc:.3f} | Pm: {pm:.3f}")
    ...
    for w_id, rate in res.workstation_utilization.items():
        print(f"    - Workstation {w_id+1} ({problem.workstations[w_id].name}): {rate*100:5.1f}%")
    ...
    SMTGanttChart.plot_static_gantt(
        ...
        output_path=gantt_file
    )
    print(f"🖼 Saved schedule Gantt chart image to: {os.path.abspath(gantt_file)}")
    
    # 5. Simple Explainable Bottleneck Analysis
    sorted_ws = sorted(res.workstation_utilization.items(), key=lambda x: x[1], reverse=True)
    b_id, b_rate = sorted_ws[0]
    print("\n======================================================================")
    print("      🧠 EXPLAINABLE OPTIMIZATION HIGHLIGHTS")
    print("======================================================================")
    print(f"  • Bottleneck Stage: Workstation {b_id+1} ({problem.workstations[b_id].name}) has the highest utilization rate at {b_rate*100:.1f}%.")
    ...
    if tardy_jobs_count == 0:
        print("  • Schedule Quality: Excellent! All job order due dates have been fully met.")
    else:
        print(f"  • Schedule Quality: Good. {tardy_jobs_count} job orders experienced tardiness due to material/capacity constraints.")
        print("    Refer to the Streamlit app's 'Solution Explainer' tab for a detailed breakdown of each tardy job.")
    print("======================================================================\n")
