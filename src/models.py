...
class Job:
    ...
    unit_processing_times: np.ndarray # Array of shape (M,) defining unit processing time at each workstation stage
...
class Batch:
    ...
    batch_index: int
    quantity: int
    processing_times: np.ndarray    # Array of shape (M,) scaled by batch quantity (unit_processing_times * quantity)
...
class ScheduleEntry:
    ...
    setup_time: float
    setup_cost: float
    waiting_time: float              # Waiting time (delay from material arrival or transport transit)
