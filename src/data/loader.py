...
class SMTDataLoader:
    ...
    def load_mat_problem(file_path: str, seed: int = 42) -> ProblemInstance:
        ...
        file_hash = int(hashlib.md5(os.path.basename(file_path).encode('utf-8')).hexdigest(), 16) % 10000
        ...
        jobs = []
        setup_cost_factor = SMT_PARAMETERS.get("Setup_Cost_Factor", 1.5)
        eligibility_density = SMT_PARAMETERS.get("Machine_Eligibility_Density", 0.8)
